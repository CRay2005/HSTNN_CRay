...
parser.add_argument('--mode', type=str, default='test2', help='type of operation (train1, test1, train2, test2)')
# Activate parameters
...
parser.add_argument('--ratio', type=float, default=0.3, help='snn ratio of the hybird network')
# Default parameters
...
def evaluate(arg_model, data_source):
    ...
    with torch.no_grad():
        for i in range(0, data_source.size(0) - 1, args.bptt):
            ...
            # 解码并打印 data（第一个样本）
            data_words = [corpus.dictionary.idx2word[idx] for idx in data[:, 0].tolist()]
            print("\nInput data:", " ".join(data_words))

            output, hidden = arg_model(data, hidden)

            # 解码并打印 output（第一个样本的预测）
            pred_indices = output.argmax(dim=-1)[:, 0].tolist()
            pred_words = [corpus.dictionary.idx2word[idx] for idx in pred_indices]
            print("Model predictions:", " ".join(pred_words))
