...
class HessianPruner:
        ...
        def _compute_hessian_importance(self, dataloader, criterion, device, is_loader, n_v=300):
            ...
            if flag == 2:
                ...
                # 设定目标保留率 - 可以根据需要调整
                target_rnn_retention = 0.25  # 保留40%的RNN神经元
                target_snn_retention = 0.5  # 保留20%的SNN神经元
                
                print(f"🎯 目标保留率: RNN={target_rnn_retention:.1%}, SNN={target_snn_retention:.1%}")
                
                # 2.1 使用top_k_direct方法分析RNN
                rnn_threshold= self._analyze_importance_distribution(
                    sorted_rnn_list, 'top_k_direct', 'RNN', target_rnn_retention
                )
                
                # 2.2 使用top_k_direct方法分析SNN
                snn_threshold= self._analyze_importance_distribution(
